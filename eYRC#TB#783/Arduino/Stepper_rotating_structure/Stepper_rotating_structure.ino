...
*/
#include <Stepper.h> 
char ss[4]={'r','b','y','g'};//ss is an array the stores the structure sequence.Position 0 is north,1 east,2 south,3west.'r' is red 'y' yellow 'b' blue 'g'green
char color;//variable to store character recieved from xbee
int index=0;//this variable stores the index of color in ss[]
Stepper myStepper(200,4,5,6,7);// initialize the stepper library on pins 4 through 7;stepper has 200 steps per revolution
/*Function Name:setup
Input:None
Output:None
...
if(index==0)
myStepper.step(0);//since 0 is north so the present sequence has the desired color already in north
if(index==1)
myStepper.step(50);//since 1 is east so the present sequence has the desired color towards east
if(index==2)
myStepper.step(100);//since 2 is south so the present sequence has the desired color towards south
if(index==3)
 myStepper.step(150);//since 3 is west so the present sequence has the desired color towards west
